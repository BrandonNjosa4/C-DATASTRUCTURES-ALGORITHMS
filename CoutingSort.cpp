...
void CountingSort(int input_array[], int s, int r)
{
    ...
    //cummalitive count of count array to get the positions of elements to be stored in the output array
    for (int i = 1; i < r; i++)
    {
        count_array[i] = count_array[i] + count_array[i - 1];
    }
    //placeing input array elements into output array in proper positions such that the result is a sorted array in Ascending order
    for (int i = 0; i < s; i++)
    {
